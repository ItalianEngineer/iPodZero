...
void sendPacket() {
    ...
    // Translate wheel rotation to ncmpcpp commands
    if (wheelPosition != lastPosition) {
        if (wheelPosition > lastPosition) {
            // Rotate clockwise (e.g., "next track" command)
            system("ncmpcpp next");
        } else {
            // Rotate counter-clockwise (e.g., "previous track" command)
            system("ncmpcpp prev");
        }
        lastPosition = wheelPosition;
    }

    // Translate button presses to ncmpcpp commands
    if (buffer[BUTTON_INDEX] == CENTER_BUTTON_BIT && buffer[BUTTON_STATE_INDEX] == 1) {
        system("ncmpcpp toggle");
    } else if (buffer[BUTTON_INDEX] == LEFT_BUTTON_BIT && buffer[BUTTON_STATE_INDEX] == 1) {
        system("ncmpcpp prev");
    } else if (buffer[BUTTON_INDEX] == RIGHT_BUTTON_BIT && buffer[BUTTON_STATE_INDEX] == 1) {
        system("ncmpcpp next");
    } else if (buffer[BUTTON_INDEX] == UP_BUTTON_BIT && buffer[BUTTON_STATE_INDEX] == 1) {
        system("ncmpcpp prev");
